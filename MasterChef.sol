...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BYIELDs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BYIELDs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BYIELDs distribution occurs.
        uint256 accByieldPerShare;   // Accumulated BYIELDs per share, times 1e12. See below.
    ...
    // BYIELDs tokens created per block.
    ...
    address public feesProcessorAddress;
    ...
    event UpdateEmissionRate(address indexed user, uint256 byieldPerBlock);
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        if(byield.totalSupply() >= MAX_SUPPLY) {
            return 0;
        } else {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 byieldReward = multiplier.mul(byieldPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        byield.mint(address(this), byieldReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                uint256 buybackShare = depositFee.div(100).mul(40);
    ...
    function updateEmissionRate(uint256 _byieldPerBlock) public onlyOwner {
        massUpdatePools();
        byieldPerBlock = _byieldPerBlock;
        emit UpdateEmissionRate(msg.sender, _byieldPerBlock);
    }
}